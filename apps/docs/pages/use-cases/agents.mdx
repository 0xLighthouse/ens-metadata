...
Agents registered with ERC-8004 or other on-chain registries can advertise those registrations on their ENS name using the `registrations` array field. Each entry follows CAIP-29 format (ERC-8004 uses ERC-721 tokens) `{namespace}:{chainId}/{tokenStandard}:{identityRegistry}/{agentId}`:
