...
The `@ens-node-metadata/cli` package provides command-line tools for managing ENS node metadata, ERC-8004 registration files, and the on-chain identity registry.

## Install

```bash
pnpm add -g @ens-node-metadata/cli
```
...
pnpm dlx @ens-node-metadata/cli --help
```
