...
3. **Pick the ENS name (or subname) the agent will be known by.** This might be a top-level name like `myagent.eth`, or a dedicated subname like `agent.alice.eth`. Subnames are recommended when an operator runs multiple agents, since each subname gets its own independent set of records.

## Step 2: Add your agent information

1. Open [ensmetadata.app](https://ensmetadata.app), connect the wallet that controls the ENS name you want to use, and navigate to the node you chose in Step 1.
