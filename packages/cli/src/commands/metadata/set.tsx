...
import { metadataWriter, validateMetadataSchema } from '@ens-node-metadata/sdk'
...
export default function SetMetadata({ args: [ensName, payloadFile], options }: Props) {
  const state = useCommand([ensName, payloadFile, options], async (setState) => {
    ...
    try {
      ...
      setState({
        status: 'done',
        message: `Transaction submitted: https://etherscan.io/tx/${result.txHash}`,
      })
    } catch (err) {
