...
export function validateSchema(value: unknown): SchemaValidationResult {
  ...
  if (typeof s.title !== 'string' || s.title.length === 0) {
    errors.push({ path: 'title', message: 'Must be a non-empty string identifying the entity described by the schema' })
  }

  if (typeof s.description !== 'string' || s.description.length === 0) {
    errors.push({ path: 'description', message: 'Must be a non-empty string explaining the organizational role of nodes using this schema' })
  }
  ...
  if (typeof s.properties !== 'object' || s.properties === null || Array.isArray(s.properties)) {
    ...
    for (const [key, attr] of Object.entries(props)) {
      if (!KEBAB_CASE_RE.test(key)) {
        errors.push({
          path: `properties.${key}`,
          message: 'Key name must be kebab-case (lowercase, hyphen-delimited, with optional dot namespacing)',
        })
      }
  ...
  const propertyKeys = typeof s.properties === 'object' && s.properties !== null && !Array.isArray(s.properties)
  ...
  if (s.patternProperties !== undefined) {
    if (typeof s.patternProperties !== 'object' || s.patternProperties === null || Array.isArray(s.patternProperties)) {
